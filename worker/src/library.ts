...
interface LibEnv {
  ...
  GALAXY_LAYOUT?: string;      // "auto" (per-device) | "circle" | "islands" — galaxy default layout
...
export async function handleLibrary(request: Request, env: LibEnv, ctx: ExecutionContext): Promise<Response> {
  ...
  if (path === "/api/library/ping") {
    ...
    // galaxyLayout is omitted unless an admin pins it; absent => the SPA picks per device.
    const gl = env.GALAXY_LAYOUT === "circle" || env.GALAXY_LAYOUT === "islands" ? env.GALAXY_LAYOUT : undefined;
    return json({ ok: true, edits: env.EDITS_ENABLED === "true", galaxyLayout: gl, identity: { kind: provider.kind, user } });
  }
