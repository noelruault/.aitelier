...
**Warning: Brotli static-dictionary mismatch on JSON / non-web-text.** Brotli's RFC
7932 Appendix A 120 KB static dictionary is HTML/JS-tuned. On JSON, log lines,
telemetry payloads, and other non-web-text corpora, low-quality Brotli (`brotli -1`)
can produce **larger** output than `gzip -6`. Confirmed empirically on a JSON API
corpus: `brotli -1` returned 167,376 wire bytes vs `gzip -6` at 150,251 (+11.4% worse).
If the corpus is JSON-heavy or non-Latin text, prefer `zstd` over low-quality Brotli.
Reserve Brotli for static, build-time encoding at quality ≥ 5 where the encoder is
strong enough to overcome the dictionary mismatch.
...
Emit `bench/score.py` once. It outputs both absolute byte-deltas AND percentage deltas
relative to baseline, plus a per-item array. Absolute numbers without scale are hard
to read ("[CI -129275, -26591]" tells you nothing about whether the delta is large);
the percentage column fixes that.
...
That overhead is invisible at compression ratios; it dominates timings for files
<50 KB or encoders/decoders that finish in <5 ms. Using the `hyperfine -- sh -c` path
for everything will pick the wrong winner on small payloads: encode times saturate to
the shell-startup floor and the score formula cannot differentiate between levels
(e.g. zstd level 3 wins over level 9 on a JSON API workload not because it is faster
but because both fell below the noise floor).

Emit a Python timer that fork/execs the encoder directly via `subprocess.run` and times
with `time.perf_counter_ns`. **Use this for everything below the 5 ms / 50 KB
boundary; use `hyperfine` above it.** Output schema matches the items.json shape used
by `score.py`.
...
When this is the right tool: encode/decode below ~5 ms per iteration, or input below
~50 KB. Use `hyperfine` above those thresholds.
...
- Per-iteration cost < 5 ms or input < 50 KB → use `bench/measure.py` (Section 4.5)
  to avoid the ~6 ms `sh -c` startup floor that obscures real encode/decode CPU.

For most JSON API and small-asset workloads, `measure.py` is the right tool. For large
JS bundles encoded at brotli-11, `hyperfine` is fine.
