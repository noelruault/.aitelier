  ...
  all.forEach(req => {
    ...
    card.innerHTML = `
      <div class="req-head flex items-center justify-between mb-1.5">
        ...
        <div class="req-meta font-mono text-[10.5px] text-ink-mute tracking-[.12em] uppercase"><span class="req-by" title="${escapeHtml(req.byId || "local draft")}">${escapeHtml(req.by || "[local]")}</span> · ${escapeHtml(new Date(req.createdAt).toISOString().slice(0,16).replace("T"," "))} · ${escapeHtml(req.entityType)} · ${escapeHtml(req.status)}</div>
      </div>
