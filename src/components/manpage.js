...
import { parsePromptManpage } from "../lib/parse-manpage-prompt.js";
...
import { renderMarkdown } from "../lib/parse-markdown.js";
...
export function renderDocPane(entity, state) {
  ...
  head.innerHTML = `
    ...
    <p class="doc-sub text-ink-soft font-body text-[16.5px] leading-[1.55] m-0 mb-[22px] max-w-[60ch]">${escapeHtml(entity.description || "")}</p>
...
export function topActionsFor(entity) {
  ...
  if (entity.type === "prompts" && isMultiStep(entity)) {
    ...
    html += `<button class="${BTN_CLAY}" data-act="copy">Copy prompt</button>`;
  } else if (entity.type === "skills") {
    html += `<button class="${BTN_CLAY}" data-act="copy">Copy slash</button>`;
  } else if (entity.type === "agents") {
    html += `<button class="${BTN_CLAY}" data-act="copy">Copy spawn syntax</button>`;
  } else if (entity.type === "hooks") {
    html += `<button class="${BTN_CLAY}" data-act="copy">Copy settings.json snippet</button>`;
  }
  // Edit / Fork / Request edit / Duplicate / Delete are always available.
  // Entries persist to localStorage on Pages, to R2 once the Worker write
  // path lands (Phase 4). CLAUDE.md rule #5 originally gated these on
  // capability; lifted per explicit user preference.
...
export function renderPromptSingle(pane, entity) {
  const parsed = parsePromptManpage(entity.body || "");
  appendParsedSections(pane, parsed);
...
export function renderPromptMultiStep(pane, entity) {
      ...
      ${entity.steps.map((s, i) => `
        <li class="flex items-center gap-3.5 px-[18px] py-4 bg-paper-deep border border-rule rounded-sm flex-wrap">
          ...
          <div class="wf-body flex-1 min-w-0">
            ...
            <div class="wf-hint font-body text-sm text-ink-soft mt-1 italic">${escapeHtml(parsePromptManpage(s.body).intro || "Step " + (i + 1))}</div>
          </div>
  ...
  entity.steps.forEach((s, i) => {
    ...
    const parsed = parsePromptManpage(s.body);
    appendParsedSections(sub, parsed);

    const raw = document.createElement("section");
    raw.className = "step-block-raw mt-6 scroll-mt-6";
    raw.innerHTML = `
      <div class="raw-head flex items-center justify-between mb-2.5 gap-3 flex-wrap">
        <h3 class="section-h font-sans text-[12px] tracking-[.32em] uppercase text-ink font-bold m-0 mb-3.5 relative pb-2.5">Raw prompt, step ${i + 1}</h3>
...
export function appendParsedSections(pane, parsed) {
  const SECTION_BODY = "section-body font-body text-[17px] leading-[1.65] text-ink";
  // .doc ol/ul utilities (font-body 16px line-height 1.62, pl-6.5, mt-2.5, gap mb-2.5)
  const LIST = "pl-[26px] mt-2.5 font-body text-base leading-[1.62] [&_li]:mb-2.5";
  const HALT = "halt mt-2.5 px-5 py-4 border-l-[3px] border-accent bg-paper-deep text-ink font-body italic text-[16.5px] rounded-r-sm";
  if (parsed.role) pane.appendChild(makeSection("Role", `<div class="${SECTION_BODY}"><p>${escapeHtml(parsed.role)}</p></div>`));
  if (parsed.intro) pane.appendChild(makeSection("Description", `<div class="${SECTION_BODY}"><p>${escapeHtml(parsed.intro)}</p></div>`));
  if (parsed.inputs.length) pane.appendChild(makeSection("Inputs (the prompt will ask you)",
    `<ol class="input-list ${LIST}">${parsed.inputs.map(i => `<li>${escapeHtml(i)}</li>`).join("")}</ol>`));
  if (parsed.execution.length) pane.appendChild(makeSection("Execution",
    `<ol class="exec-list ${LIST}">${parsed.execution.map(s => `<li>${escapeHtml(s)}</li>`).join("")}</ol>`));
  if (parsed.halt) pane.appendChild(makeSection("Stops at", `<div class="${HALT}">${escapeHtml(parsed.halt)}</div>`));
  if (parsed.conventions.length) pane.appendChild(makeSection("Conventions",
    `<ul class="${LIST}">${parsed.conventions.map(c => `<li>${escapeHtml(c)}</li>`).join("")}</ul>`));
}
...
export function renderSkillBody(pane, entity) {
  ...
  pane.appendChild(makeSection("Metadata", metaTable(vm.meta)));
...
export function renderAgentBody(pane, entity) {
  ...
  pane.appendChild(makeSection("Spawn syntax",
    `<pre class="prompt-raw m-0 px-[26px] py-6 bg-ink text-paper font-mono text-[13px] leading-[1.7] rounded-sm overflow-x-auto whitespace-pre-wrap break-words border border-ink">${escapeHtml(vm.spawn)}</pre>`));
  pane.appendChild(makeSection("Metadata", metaTable(vm.meta)));
  pane.appendChild(makeSection("Description", `<div class="section-body md-body font-body text-[17px] leading-[1.65] text-ink">${renderMarkdown(vm.body)}</div>`));
...
export function renderHookBody(pane, entity) {
  ...
  // Structured view of hook.json. Renders first when available so the
  // user gets the semantic decomposition above the raw JSON.
