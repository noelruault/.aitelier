...
import { currentUser } from "../lib/attribution.js";
...
export function saveRequest() {
  ...
  if (reqModalState.mode === "edit") {
    ...
    // Stamp the requester when an identity provider is live (§5). On Pages /
    // anonymous there's no user, so `by` stays unset and the list shows
    // "[local]" to mark the draft as a local-only scratchpad entry (§10).
