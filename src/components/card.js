...
import { renderSpawnSyntax } from "../lib/parse-manpage-agent.js";
 ...
 * Click the Copy button -> copies the body / slash / spawn syntax. */
...
export function renderCard(entity, opts = {}) {
  ...
  card.innerHTML = `
    ...
    <div class="card-body relative ${bodyPad}" style="--cat-color:${catVar}">
      ...
      ${midBlock}
      <div class="card-foot ${footPad} border-t border-rule-soft flex justify-between items-center">
        <span class="meta font-mono text-[10.5px] text-ink-mute tracking-[.08em] uppercase">${cardMeta(entity)}</span>
        <span class="copy-group inline-flex gap-1.5">${footAction}</span>
      </div>
    </div>
  `;
...
export function footButtonForType(entity) {
  ...
  if (entity.type === "skills") {
    return `<button class="${COPY}" data-act="copy">Copy slash</button>`;
  }
  if (entity.type === "agents") {
    return `<button class="${COPY}" data-act="copy">Copy spawn</button>`;
  }
  if (entity.type === "hooks") {
    return `<button class="${COPY}" data-act="copy">Copy snippet</button>`;
  }
...
/* Copy the right content per type. */
export function copyEntity(entity) {
  const label = humanizeName(entity.name);
  if (entity.type === "prompts") {
    if (isMultiStep(entity)) return copyStep(entity, 0);
    return copyText(entity.body || "", label);
  }
  if (entity.type === "skills") return copyText(entity.slash || "", `${label} slash`);
  if (entity.type === "agents") return copyText(renderSpawnSyntax(entity), `${label} spawn`);
  if (entity.type === "hooks") {
    const { snippet } = parseHookManpage(entity);
    return copyText(snippet || entity.body || "", `${label} snippet`);
  }
}
