...
export function renderGalaxy(mount, opts) {
  ...
  mount.innerHTML = `
    <div class="galaxy-frame bg-paper border border-rule rounded-md px-3 pt-3 pb-4">
      <div class="galaxy-wrap overflow-x-auto md:overflow-visible touch-pan-x [-webkit-overflow-scrolling:touch]">
        <svg class="galaxy-svg w-full h-auto block font-sans min-w-[960px] md:min-w-0" viewBox="0 0 ${GALAXY_W} ${GALAXY_H}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="Entity galaxy">
  ...
  const wrap = mount.querySelector(".galaxy-wrap");
  if (wrap) {
    const center = () => {
