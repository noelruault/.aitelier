...
function groupOne(folder, paths, sizeOf) {
  ...
  for (const path of paths) {
    ...
    if (rest === main) {
      ent.main = mainKey;
    } else if (rest !== "aitelier.json" && !rest.endsWith(".history.jsonl") && rest !== "history.jsonl") {
      // Server-derived sidecars/logs surface in the Metadata section, not
      // as attachments (mirrors isServerDerivedPath in worker/src/library.ts).
