...
const FALSE = Object.freeze({ edits: false, identity: NONE_IDENTITY, galaxyLayout: undefined });
let cached = FALSE;
let primed = false;

// Galaxy default layout advertised by the Worker env (GALAXY_LAYOUT). Only the two
// known values pass through; anything else (incl. absent) is undefined so the galaxy
// falls back to its own default. The per-tab A/B switch overrides this.
function normLayout(v) { return v === "circle" || v === "islands" ? v : undefined; }
...
export async function primeCapabilities() {
  ...
  try {
    ...
    if (raw) {
      ...
      if (parsed && typeof parsed === "object") {
        cached = Object.freeze({ edits: !!parsed.edits, identity: normalizeIdentity(parsed), galaxyLayout: normLayout(parsed.galaxyLayout) });
        return cached;
      }
    }
  ...
  try {
    ...
    if (r.ok && ct.includes("application/json")) {
      ...
      cached = Object.freeze({ edits: !!(j && j.edits), identity: normalizeIdentity(j), galaxyLayout: normLayout(j && j.galaxyLayout) });
    }
