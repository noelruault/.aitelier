...
function buildBase({ meta }, fallbackSlug, sidecar) {
  ...
  // Write-attribution passthrough (.plans/0008_user-management.md §8). Written to
  // the sidecar server-side by the Worker; the renderer reads them for the
  // "edited by" eyebrow / contributors. Absent on Pages and pre-v1 sidecars.
  if (side.created_by !== undefined) out.created_by = side.created_by;
  if (side.last_updated_by !== undefined) out.last_updated_by = side.last_updated_by;
  if (Array.isArray(side.contributors)) out.contributors = side.contributors;
  return out;
}
...
export function buildHook(parsed, fallbackSlug, sidecar, prose) {
  ...
  if (rawSrc && tryParseHookJson(rawSrc)) {
    ...
    // Write-attribution passthrough (§8), same as buildBase for the other types.
    if (sidecar && sidecar.created_by !== undefined) e.created_by = sidecar.created_by;
    if (sidecar && sidecar.last_updated_by !== undefined) e.last_updated_by = sidecar.last_updated_by;
    if (sidecar && Array.isArray(sidecar.contributors)) e.contributors = sidecar.contributors;
